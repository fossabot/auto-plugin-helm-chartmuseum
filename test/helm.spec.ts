...
describe(Helm.name, () => {
  ...
  describe('prepChart', () => {
    ...
    it('works', async () => {
      await helm.prepChart('src','dest','1234',{recursive: true, replaceFileWithRepository: true, replaceVersionToken: true, repository: 'testrepo'})
      expect(exec).toBeCalledWith('helm',['dep','up','src'])
      expect(exec).toBeCalledWith('helm',['package','src','-d','dest'])

      expect(rm).toBeCalledWith('src/charts/*.tgz',{force: true, recursive: true})
    ...
    it('replaces correct repository', async () => {
      ...
      jest.spyOn(helm,'inlineReplace').mockImplementation(async (path, replacers) => {
        replacerFn(replacers(`test:  file://../some/path\nkey2:     file://../../otherPath`))
      })
      await helm.prepChart('src','dest','1234',{recursive: true, replaceFileWithRepository: true, replaceVersionToken: true, repository: 'testrepo'})
      expect(replacerFn).toBeCalledTimes(1)
      expect(replacerFn).toBeCalledWith(`test:  'testrepo'\nkey2:     'testrepo'`)
    })

    it('does not replace repository if disabled', async () => {
      await helm.prepChart('src','dest','1234',{recursive: true, replaceFileWithRepository: false, replaceVersionToken: true, repository: 'testrepo'})
      expect(helm.inlineReplace).not.toBeCalled()
    })

    it('does not replace repository if repository token not provided', async () => {
      await helm.prepChart('src','dest','1234',{recursive: true, replaceFileWithRepository: true, replaceVersionToken: true})
      expect(helm.inlineReplace).not.toBeCalled()
    })
  })
  ...
  describe('prepCharts', () => {
    ...
    it('works', async () => {
      ...
      expect(helm.prepChart).toHaveBeenCalledWith("dest/test1", "dest", "1234", {recursive: true, replaceFileWithRepository: true, replaceVersionToken: true})
      expect(helm.prepChart).toHaveBeenCalledWith("dest/test2", "dest", "1234", {recursive: true, replaceFileWithRepository: true, replaceVersionToken: true})
    ...
    it('does not replace version when disabled', async () => {
      await helm.prepCharts('1234','src','dest',{replaceVersionToken: false})
    ...
    it('uses helm docs when enabled', async () => {
      ...
      await helm.prepCharts('1234','src','dest')
      expect(exec).toBeCalledWith('helm-docs',['-u','publish','-s','alphanum'])
    })

    it('replaces correct version', async () => {
      const replacerFn = jest.fn()
      jest.spyOn(helm,'inlineReplace').mockImplementation(async (path, replacers) => {
        replacerFn(replacers(`${helm.options.versionToken.toLowerCase()}\ntest\n${helm.options.versionToken.toUpperCase()}`))
      })
      await helm.prepCharts('1234','src','dest')
